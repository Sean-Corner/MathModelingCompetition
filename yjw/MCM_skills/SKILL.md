...
| 3 | `03-coding-visual` | 实现代码、运行实验、保存结果，并生成**草稿版**数据图（matplotlib / seaborn 等探索用图）。                                                 | `code/`, `results/`, `reports/RESULTS_REPORT.md`, `figures/*.pdf` |
...
- **双轨图表**：`03-coding-visual` 与 `04-drawio` 只负责**草稿图**（落在 `figures/`）；论文级高保真图表由 `07-graph-optimization` 重新生成（落在 `figures/antv/`），**不在 03/04 阶段提前出成品**。
