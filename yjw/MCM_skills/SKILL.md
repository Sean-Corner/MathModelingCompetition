...
| 4 | `04-drawio`            | 绘制论文级非数据示意图（技术路线图/流程图/模型结构图，scibox-diagram 方法）。 | `figures/*.drawio`, `figures/*.png`, `reports/DRAWIO_REPORT.md` |
...
- 图表模板库：`03-coding/scibox-figure/`（数据图模板库，用于 03 的 Step 4）；`04-drawio/scibox-diagram/`（示意图框架，用于 04 的 Step 3）。详细规约由各自 `SKILL.md` 提供。
...
├── code/
├── results/
├── figures/                       # 03/04 论文级成品图（.png / .drawio）
└── paper/
