...
**职责边界**：只换"皮"，不重算"数"。任何对数据/模型的修改都属于上游 02/03 步骤，07 不接受。
...
| 触发方式 | 场景 | 说明 |
| --- | --- | --- |
| **手动调用** | 用户说"升级图表 / 出终稿图 / 用 antv 重画 / 跑 07" | 立刻执行 |
| **自动追加** | `06-verity` 通过 + `todo.md` 标记 `auto-antv: true` | 06 末尾自动追加，无需人工 |
| **不触发** | 数据或模型仍在迭代（02/03 未稳定） | 强行升级浪费 token |

> ⚠️ 触发前先确认 `reports/RESULTS_REPORT.md` 是终版；如果该报告里还有 TODO，07 必须先停下，让用户回到 02/03。
...
| 草稿论文 | `paper/竞赛论文(草稿).pdf` 或 `.typ` | 替换图表引用 |

### 输出

| 产物 | 路径 | 说明 |
| --- | --- | --- |
| AntV 高保真图 | `figures/antv/*.pdf` | 终稿用图，300 DPI |
| AntV 工作日志 | `reports/ANTV_REPORT.md` | 每张图的输入数据、模板、配色、ANTV 子 skill 名 |
| 终稿论文 | `paper/竞赛论文.pdf` | 替换所有图表引用后的最终 PDF |
| 终稿论文源 | `paper/竞赛论文.typ`（如使用 Typst） | 含图片路径替换记录 |
...
- [ ] **PDF 可读**：`paper/竞赛论文.pdf` 能正常打开，无乱码、无缺字符；文件大小合理（一般 2~10 MB）。
...
- 上层 `SKILL.md` 的产物结构里 `figures/antv/`、`reports/ANTV_REPORT.md`、`paper/竞赛论文.pdf` 三项由本 skill 产出；其他文件不要在本 skill 里改动。
- 触发条件（手动 / `06-verity` 通过后自动追加）由上层 `todo.md` 控制，本 skill 只负责执行。