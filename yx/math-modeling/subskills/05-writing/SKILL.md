...
撰写第五部分“模型的建立与求解”前，必须先读取 `references/model-establishment-solving.md`。该文件提供第五部分的推荐结构、详细演算要求、常见模型类型的写法和示例；写作时只能将其作为结构与表达参考，具体模型、公式、参数、数值和结论必须依据当前题目及项目已有报告，不得套用示例或自行编造。
...
第五部分“模型的建立与求解”是论文正文的核心，应针对不同问题分别设为小节。每个问题都应完整呈现“问题目标与变量定义—模型假设与建立—公式及目标函数/约束条件推导—求解算法与实施步骤—结果与检验”的逻辑链条。行文中必须包含数学模型建立与求解的详细演算过程，以及所采用算法的具体步骤；不得只罗列模型名称、软件名称或最终结果。推导目标函数、约束条件和模型公式时，必须定义符号、说明推导依据并保持前后一致；所有公式、算法、参数和结论必须经过检查且完全正确。
